...
describe('App', () => {
  ...
  it('check if GBPValue input working', async () => {
    ...
    GBPInput.props.onChange({ target: { value: 100.00 } });
    await waitFor(() => {
      expect(GBPInput.props.value).toBe(100.00);
    });
  });
  ...
  it('check if PLNValue input working', async () => {
    ...
    PLNInput.props.onChange({ target: { value: 100.00 } });
    await waitFor(() => {
      expect(PLNInput.props.value).toBe(100.00);
    });
  });

  it('updates GBPValue when ZlotyValue changes', async () => {
    let expectedValue = (100.00/rate.toFixed(2)).toFixed(2).toString()
    const component = renderer.create(<App />);
    const PLNInput = component.root.findByProps({ id: 'pln' });
    PLNInput.props.onChange({ target: { value: 100.00 } });
    await waitFor(() => {
  ...
  it('updates ZlotyValue when GBPValue changes', async () => {
    let expectedValue = (10.00*rate.toFixed(2)).toFixed(2).toString();
    const component = renderer.create(<App />);
    const GBPInput = component.root.findByProps({ id: 'gbp' });
    GBPInput.props.onChange({ target: { value: 10.00 } });
    await waitFor(() => {
      const PLNInput = component.root.findByProps({ id: 'pln' });
